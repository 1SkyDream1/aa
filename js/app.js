	...
	(() => {
		...
		function extend(target, src) {
			...
			Object.keys(src).forEach((key) => {
				...
				else if (ssr_window_esm_isObject(src[key]) && ssr_window_esm_isObject(target[key]) && Object.keys(src[key]).length > 0) extend(target[key], src[key]);
			});
		}
		const ssrDocument = {
			body: {},
		...
		function utils_elementOuterSize(el, size, includeMargins) {
			...
			if (includeMargins)
				return (
					el[size === "width" ? "offsetWidth" : "offsetHeight"] + parseFloat(window.getComputedStyle(el, null).getPropertyValue(size === "width" ? "margin-right" : "margin-top")) + parseFloat(window.getComputedStyle(el, null).getPropertyValue(size === "width" ? "margin-left" : "margin-bottom"))
				);
				...
				if (
					...
					data.currentTranslate >
						(params.centeredSlides
							? swiper.minTranslate() - swiper.slidesSizesGrid[swiper.activeIndex + 1] - (params.slidesPerView !== "auto" && swiper.slides.length - params.slidesPerView >= 2 ? swiper.slidesSizesGrid[swiper.activeIndex + 1] + swiper.params.spaceBetween : 0) - swiper.params.spaceBetween
							: swiper.minTranslate())
				)
					swiper.loopFix({
						direction: "prev",
					...
					data.currentTranslate <
						(params.centeredSlides
							? swiper.maxTranslate() +
								...
								(params.slidesPerView !== "auto" && swiper.slides.length - params.slidesPerView >= 2 ? swiper.slidesSizesGrid[swiper.slidesSizesGrid.length - 1] + swiper.params.spaceBetween : 0)
							: swiper.maxTranslate())
				)
					swiper.loopFix({
						direction: "next",
		...
		class swiper_core_Swiper {
			...
			getSlideClasses(slideEl) {
				...
				return slideEl.className
					.split(" ")
					.filter((className) => className.indexOf("swiper-slide") === 0 || className.indexOf(swiper.params.slideClass) === 0)
					.join(" ");
			}
			emitSlidesClasses() {
				const swiper = this;
		...
		function initSliders() {
			...
			if (document.querySelector(".slider-documents"))
				new swiper_core_Swiper(".slider-documents", {
					modules: [Navigation],
					observer: true,
					observeParents: true,
					slidesPerView: 5,
					...
					speed: 800,
					navigation: {
						prevEl: ".navigation-slider-documents__icon--left",
						nextEl: ".navigation-slider-documents__icon--right",
					},
					breakpoints: {
						0: {
							slidesPerView: 2,
		...
		class ScrollWatcher {
			...
			getScrollWatcherConfig(paramsWatch) {
				...
				else if (paramsWatch.root !== "null") this.scrollWatcherLogging(`Эмм... родительского объекта ${paramsWatch.root} нет на странице`);
		...
		let popupTexts = {};
		fetch("../files/popupTexts.json")
			.then((response) => response.json())
			.then((data) => {
				popupTexts = data;
				console.log("Загруженный объект popupTexts:", popupTexts);
				document.querySelectorAll("[data-popup]").forEach((item) => {
					item.addEventListener("click", function (event) {
						event.preventDefault();
						const title = this.querySelector(".pop-title")
							.innerText.trim()
							.replace(/(\r\n|\n|\r)/g, " ");
						document.querySelector(".info-pop__title").innerText = title;
						document.querySelector(".info-pop__text").innerHTML = popupTexts[title] || "Текст не найден.";
